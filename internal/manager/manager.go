...
// New creates a Manager watching the given bin directory.
...
func (m *Manager) discover() error {
	bins, errs := discovery.Scan(m.ctx, m.binDir)
...
func (m *Manager) checkForChanges() {
	...
	for _, entry := range entries {
		...
		m.mu.Lock()
		lastMod, known := m.modTimes[path]
		m.mu.Unlock()

		if !known || info.ModTime().After(lastMod) {
			slog.Debug("detected change", "file", entry.Name())
			m.handleBinaryChange(path)
		}
	}
}

func (m *Manager) handleBinaryChange(path string) {
	// Run hello on the changed binary.
	bins, errs := discovery.Scan(m.ctx, filepath.Dir(path))
	for _, err := range errs {
		slog.Warn("re-discovery warning", "error", err)
	}
	...
	for _, bin := range bins {
		...
		m.mu.Lock()
		existing, running := m.processes[id]
		m.mu.Unlock()

		if running {
			slog.Info("stopping for restart", "id", id)
			existing.Shutdown(shutdownTimeout)
			m.mu.Lock()
		...
		info, _ := os.Stat(path)
		if info != nil {
			m.modTimes[path] = info.ModTime()
		}
		m.mu.Unlock()

		// Start new version.
		if err := m.startBinary(id); err != nil {
			slog.Error("failed to start updated binary", "id", id, "error", err)
		}
	}
}
